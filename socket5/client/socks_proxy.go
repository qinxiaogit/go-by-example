...
func (s *SocksProxy) handleV5() {
	...
	if ver, err := bio.ReadByte(); err != nil || ver != 5 {
	...
	if command != tcpStream {
		tslog.Red("command not supported:%v", command)
		return
	}
	...
	case addrTypeDomain:
		...
		if nameBytes, err = readn(bio.Reader, uint(nameLen)); err != nil {
			tslog.Red("red domain error:%v", err)
			return
		}
		strAdd = string(nameBytes)
	default:
	...
	hostport := fmt.Sprintf("%s:%d", strAdd, port)
	tslog.Log("端口:%s,%v,%d", hostport, portArray, port)
...
func (s *SocksProxy) relay(host string, conn net.Conn) {
	...
	err := sr.Relay(host, conn, func(r Relayer) error {
		return r.ToLocal([]byte{5, 0, 0, 1, 0, 0, 0, 0, 0})
	})
	if err != nil {
